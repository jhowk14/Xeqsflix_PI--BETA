...
const App = () => {
  ...
  const selectPainel = useCallback(() => {
    ...
    <div className="flex items-center h-full justify-center">
      <div className="flex flex-col">
        ...
        <div className="flex items-center justify-center gap-8 mt-10">
            ...
            <UserCard name={currentUser?.name == 'admin' ? 'home' : currentUser?.name} />
            </div> 
            <div onClick={() => selectPainel()}>
            {currentUser?.name === 'admin' ? <UserCard name="Adiministrativo"/> : '' } 
            </div>
        </div>
      </div>
    </div>
